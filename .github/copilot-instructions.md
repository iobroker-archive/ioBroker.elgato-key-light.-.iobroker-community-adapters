...
### Elgato Key Light Adapter Context

This adapter provides integration with **Elgato lighting devices** including:
- **Elgato Key Light** - Professional LED panel for streaming and content creation
- **Elgato Key Light Air** - Wireless version of the Key Light
- **Elgato Light Strip** - RGB LED strips for accent lighting

**Core Functionality:**
- **Device Discovery**: Automatic detection of Elgato lights on the local network
- **Real-time Control**: Adjust brightness (0-100%), color temperature (2900K-7000K for Key Lights), and RGB colors (for Light Strips)
- **Status Monitoring**: Poll device status via HTTP API calls at configurable intervals
- **Admin UI**: React-based interface for device management and control

**Technical Implementation:**
- **Connection Type**: Local network HTTP API communication
- **Polling Interval**: Configurable (default 60 seconds)
- **API Protocol**: Elgato's REST API endpoints
- **Admin Interface**: Material-UI React components with TypeScript
- **State Management**: Real-time updates of device brightness, temperature, and power states

**Unique Requirements:**
- Handle different device types (Key Lights with temperature control vs Light Strips with RGB control)
- Manage local network discovery and IP address changes
- Coordinate bulk operations (set all lights to same brightness/temperature)
- Handle device offline/online status gracefully
- Support both manual IP configuration and automatic discovery
...
### Elgato Key Light Specific Testing Patterns

**Local Network Device Testing:**
- Test adapter behavior when Elgato devices are not available on the network
- Mock HTTP responses for device discovery and control endpoints
- Test different device types (Key Light vs Light Strip) response formats
- Validate brightness/temperature value ranges and conversions

**Example Elgato Integration Test:**
```javascript
it('should handle Elgato device offline scenario gracefully', function () {
    return new Promise(async (resolve, reject) => {
        try {
            const obj = await new Promise((res, rej) => {
                harness.objects.getObject('system.adapter.elgato-key-light.0', (err, o) => {
                    if (err) return rej(err);
                    res(o);
                });
            });

            // Configure with non-existent device IP
            Object.assign(obj.native, {
                interval: 10,
                devices: [{ name: 'Test Light', ip: '192.168.1.999' }]
            });
            ...
            const connectionState = await harness.states.getStateAsync('elgato-key-light.0.info.connection');
            
            // Should be false since device is not reachable
            if (!connectionState || connectionState.val === false) {
                console.log('✅ Adapter properly handles offline devices');
                resolve(true);
            } else {
                reject(new Error('Adapter should report connection false for unreachable devices'));
            }
        } catch (error) {
            reject(error);
        }
